Feature:  Test V2 API in scope
  ...
  Background:  Check all Vx API
    ...
    * def DATERANGE = 'before=2024-01-05&after=2023-12-14'
    * def FLEETID =   'UAETSJ'
    * def DEVICE = '862798051333967'
    * def ASSET = 'JH05CN5397'
    * def DRIVERID = '_UNASSIGNED'
    * def IGNITION_ID = 'ignition_2023_06_10_03_43_26_261_862798051333967_vtly0'
    * java.lang.Thread.sleep(1000)

  @FleetTrips
  Scenario:  Test Fleet trips -Daterange
    Given url TestQAURL+'/fleets/'+FLEETID+'/trips?'+DATERANGE+'&limit=500&skip=0'
  ...
  Scenario:  Test Fleet trips - Range
    Given url TestQAURL+'/fleets/'+FLEETID+'/trips?'+'skip=0&limit=10000&'+DATERANGE
    #And header Authorization = 'Bearer ' + Token
    #And headers { "x-lm-desired-account": "machinemax",  "Content-Type": "application/json",  "Authorization": "Basic bGlnaHRtZXRyaWNzX3FhOjhOd1BxUUVBZzA0TExKZmV0UWJvZ1d5M2pkSmxkbnB3bjFrYUI5UHhseHVEbjhrUEkyMjdyMmZwcExWRExJY0Q="}
    When method GET
    Then status 200
  ...
  Scenario:   Test Fleet Aggregate -SKIP
    Given url TestQAURL+'/fleets/'+FLEETID+'/aggregate?'+'skip=0&limit=10000&'+DATERANGE
    When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test driver trips -Skip
    Given url TestQAURL+'/fleets/'+FLEETID+'/drivers/'+DRIVERID+'/trips?'+'skip=0&limit=10000&'+DATERANGE
   When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test driver Aggregate by Limit
    Given url TestQAURL+'/fleets/'+FLEETID+'/drivers/'+DRIVERID+'/aggregate?'+'skip=0&limit=10000&'+DATERANGE
    When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test Asset trips by limit
    Given url TestQAURL+'/fleets/'+FLEETID+'/assets/'+ASSET+'/trips?'+'skip=0&limit=10000&'+DATERANGE
   When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test Asset Aggregate by Limit
    Given url TestQAURL+'/fleets/'+FLEETID+'/assets/'+ASSET+'/aggregate?'+'skip=0&limit=10000&'+DATERANGE
    When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test Fleet Ongoing trips -Limit
    Given url TestQAURL+'/fleets/'+FLEETID+'/ongoing-trips?'+'limit=10000&skip=0&'+DATERANGE
    When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test Fleet Latest trips -Limit Skip
    Given url TestQAURL+'/fleets/'+FLEETID+'/latest-trips-by-asset-id'+'?limit=10000&skip=0&'+DATERANGE+'&includeInsignificantTrips=true'
    When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test Asset aggregate Violation
    Given url TestQAURL+'/fleets/'+FLEETID+'/assets/'+ASSET+'/aggregate/violations?'+DATERANGE
    When method GET/aggregate/violations
    Then status 200
    And print response

  @AggregateAssetViolation
  Scenario Outline:   Test Asset aggregate Violation GroupBy <range>
    Given url TestQAURL+'/fleets/'+FLEETID+'/assets/'+ASSET+'/aggregate/violations?'+'groupBy=<range>&'+DATERANGE
   When method GET
    Then status 200
    And print response
    Examples:
  ...
  Scenario Outline:   Test Fleet  Violation -Challenge
    ...
    Examples:
      ...
      | 2023-12-16            | 2023-11-27           | 2023-12-23              | 2023-11-27             |
      | 2023-12-23            | 2023-11-27           | 2023-12-10              | 2023-11-27             |
  ...
  @ListAssetViolation
  Scenario:   Test Asset  Violation
    Given url TestQAURL+'/fleets/'+FLEETID+'/assets/'+ASSET+'/violations?'+'skip=0&limit=10000&'+DATERANGE
    When method GET
    Then status 200
    And print response
  ...
  Scenario:   Test External Events
    ...
    Given url TestQAURL+'/fleets/'+FLEETID+'/external-events?'+DATERANGE+'&limit=100000&skip=0&sort=desc'
    When method GET
    Then status 200
  ...
  Scenario Outline:  Test eDVR Request List by sortOrder <sortOrder> and <STATUS>
    Given url TestQAURL+'/fleets/'+FLEETID+'/edvr-requests/list?status=<STATUS>&'+DATERANGE+'&sort=<sortOrder>&limit=1000000&skip=0'
    When method GET
    Then status 200
    And print response
    Examples:
